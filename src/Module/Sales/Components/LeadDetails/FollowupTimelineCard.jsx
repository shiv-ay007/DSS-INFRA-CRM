...
const FollowupTimelineCard = ({ lead }) => {
  ...
  const hasNextFollowup = !!(
    ...
    (lead?.nextFollowupDate && lead.nextFollowupDate !== "--" && lead.nextFollowupDate !== "Completed") ||
    (lead?.nextFollowup && lead.nextFollowup !== "--" && lead.nextFollowup !== "Completed")
  );

  const nextFollowupDisplay = hasNextFollowup
    ? `${lead?.nextFollowupDate || lead?.nextFollowup || "Scheduled"} ${lead?.nextFollowupTime ? `• ${lead.nextFollowupTime}` : ""}`
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/90 shadow-sm p-5 sm:p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-slate-100">
        ...
        <span
          className={`px-3.5 py-1.5 rounded-full text-xs font-mono font-extrabold flex items-center gap-1.5 shadow-2xs border ${
            hasNextFollowup
              ? "bg-amber-50 text-amber-800 border-amber-200"
              : "bg-slate-100 text-slate-500 border-slate-200"
          }`}
        >
          <FaCalendarAlt className={hasNextFollowup ? "text-amber-600 text-xs" : "text-slate-400 text-xs"} />
      ...
      <div className="space-y-4">
        ...
        {followupHistory.length > 0 ? (
          ...
          <div className="p-6 rounded-2xl bg-slate-50 border border-dashed border-slate-300 text-center space-y-1">
            <p className="text-xs sm:text-sm font-bold text-slate-600">No Follow-up Activity Logged Yet</p>
            <p className="text-xs text-slate-400">Schedule a follow-up or add a remark to start tracking client interaction history.</p>
          </div>
        )}
      </div>
    </div>
